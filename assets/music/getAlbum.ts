...
export default async function getAlbum(albumID: string, storeFront: string = "nz", catalogType: string = "albums") {
    ...
    try {
        ...
        let amSearchRaw = await fetch(`https://amp-api.music.apple.com/v1/catalog/${storeFront}/${catalogType}/${albumID}?art[url]=l&extend=editorialArtwork,editorialVideo,extendedAssetUrls,offers`, {
            method: "GET",
            headers: {
